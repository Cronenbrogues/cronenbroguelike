...
def _build_arg_strings(func):
    ...
    for parameter_kind in inspect._ParameterKind:
        if parameter_kind is inspect.Parameter.KEYWORD_ONLY and kind_to_argstrings.get(parameter_kind):
            argument_strings.append('*')
        argument_strings.extend(kind_to_argstrings.get(parameter_kind, []))
        if parameter_kind is inspect.Parameter.POSITIONAL_ONLY and kind_to_argstrings.get(parameter_kind):
            argument_strings.append('/')
...
def when(command, context=None, **kwargs):
    ...
    def decorator(func):
        ...
        exec(f"""def wrapped({arg_string}):
            with _poll_context():
                logging.debug("returning from context")
                return func({call_string})""", these_globals, these_locals)
